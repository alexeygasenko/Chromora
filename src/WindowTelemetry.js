      ...
      .addDiv({'class': 'bm-window-content'})
        ...
        .addDiv({'class': 'bm-container bm-scrollable'})
          ...
          .addDiv({'class': 'bm-container'})
            ...
            .addUl()
              .addLi({'innerHTML': `A unique identifier (UUIDv4) generated by ${escapeHTML(this.name)}. This enables telemetry without tracking your actual user ID.<br>Your UUID is: <b>${escapeHTML(this.uuid)}</b>`}).buildElement()
              .addLi({'innerHTML': `The version of ${escapeHTML(this.name)} you are using.<br>Your version is: <b>${escapeHTML(this.version)}</b>`}).buildElement()
              .addLi({'innerHTML': `Your browser type, used to determine ${escapeHTML(this.name)} outages and browser popularity.<br>Your browser type is: <b>${escapeHTML(browser)}</b>`}).buildElement()
              .addLi({'innerHTML': `Your OS type, used to determine ${escapeHTML(this.name)} outages and platform usage.<br>Your OS type is: <b>${escapeHTML(os)}</b>`}).buildElement()
              .addLi({'innerHTML': `The date and time that ${escapeHTML(this.name)} sent the telemetry information.`}).buildElement()
            .buildElement()
            .addP({'innerHTML': `All data above is <b>aggregated every hour</b>. Anything that could be considered personal is deleted from the server. Aggregated data means totals such as "42 people used ${escapeHTML(this.name)} on Google Chrome this hour", which cannot identify anyone.`}).buildElement()
          .buildElement()
        .buildElement()
      .buildElement()
