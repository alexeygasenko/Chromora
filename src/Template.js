...
export default class Template {
  ...
  async createTemplateTiles(tileSize, paletteBM, shouldSkipTransTiles, shouldAggSkipTransTiles) {
    ...
    const bitmap = await createImageBitmap(this.file); // Owned only by this operation.
    const templateTiles = {};
    try {
      ...
      const sourceImageData = context.getImageData(0, 0, imageWidth, imageHeight);
      const totalPixelMap = await this.#calculateTotalPixelsFromImageData(sourceImageData, paletteBM); // Calculates total pixels from the template buffer retrieved from the canvas context image data
      ...
      for (let pixelY = this.coords[3]; pixelY < imageHeight + this.coords[3];) {
        ...
        for (let pixelX = this.coords[2]; pixelX < imageWidth + this.coords[2];) {
          ...
          const templateTileName = `${
            (this.coords[0] + Math.floor(pixelX / this.tileSize)).toString().padStart(4, '0')},${
            (this.coords[1] + Math.floor(pixelY / this.tileSize)).toString().padStart(4, '0')},${
            (pixelX % this.tileSize).toString().padStart(3, '0')},${
            (pixelY % this.tileSize).toString().padStart(3, '0')
          }`;
      ...
      for (const tile of Object.values(templateTiles)) {tile.close?.();}
      this.chunked32 = {};
      throw error;
    } finally {
      bitmap.close?.();
  ...
  /** Returns true if a source rectangle contains a nontransparent pixel.
   * Normal mode scans original alpha exactly; only explicit aggressive mode downsamples.
   * The creation path reuses its full-image readback instead of reading every tile twice.
   */
  calculateCanvasTransparency({bitmap, bitmapParams, transCanvas, transContext, sourceImageData}) {
    const [sourceX, sourceY, width, height] = bitmapParams;
    if (this.shouldAggSkipTransTiles) {
      transCanvas.width = 10;
      transCanvas.height = 10;
      transContext.drawImage(bitmap, ...bitmapParams, 0, 0, 10, 10);
      const pixels = transContext.getImageData(0, 0, 10, 10).data;
    ...
    let pixels = sourceImageData?.data;
    let stride = sourceImageData?.width;
    let x = sourceX;
    let y = sourceY;
    if (!pixels) {
      transCanvas.width = width;
      transCanvas.height = height;
      transContext.drawImage(bitmap, ...bitmapParams, 0, 0, width, height);
      pixels = transContext.getImageData(0, 0, width, height).data;
      stride = width;
      x = 0;
      y = 0;
    }
    for (let row = y; row < y + height; row++) {
      const end = ((row * stride) + x + width) * 4;
      for (let index = ((row * stride) + x) * 4 + 3; index < end; index += 4) {
        if (pixels[index] !== 0) {return true;}
      }
    }
    return false;
  ...
  /** Returns the absolute bounding origin shared by reconstruction and migration. */
  getChunkedOrigin() {
    let minX = Infinity;
    let minY = Infinity;
    const keys = this.chunked instanceof Map ? this.chunked.keys() : Object.keys(this.chunked ?? {});
    for (const key of keys) {
      const values = String(key).split(',').map(Number);
      if ((values.length !== 4) || !values.every(Number.isFinite)) {
        throw new TypeError(`Invalid template chunk coordinates: ${key}`);
      }
      const [tileX, tileY, pixelX, pixelY] = values;
      minX = Math.min(minX, tileX * this.tileSize + pixelX);
      minY = Math.min(minY, tileY * this.tileSize + pixelY);
    }
    if (!Number.isFinite(minX) || !Number.isFinite(minY)) {
      return Array.isArray(this.coords) ? this.coords.slice() : [0, 0, 0, 0];
    }
    return [Math.floor(minX / this.tileSize), Math.floor(minY / this.tileSize), minX % this.tileSize, minY % this.tileSize];
  }

  /** Updates the origin to the independent minimum world X and Y of all chunks. */
  calculateCoordsFromChunked() {
    this.coords = this.getChunkedOrigin();
    return this.coords;
  }

  /** Releases persistent bitmap resources once the manager has released active users. */
  dispose() {
    const bitmaps = this.chunked instanceof Map ? this.chunked.values() : Object.values(this.chunked ?? {});
   ...
   * @since 0.88.6
   */
  async #calculateTotalPixelsFromImageData(imageData, paletteBM) {
    ...
    for (let pixelIndex = 0; pixelIndex < buffer32Arr.length; pixelIndex++) {
      ...
      if ((pixelIndex % 16384 === 0) && performance.now() - workSliceStarted >= 4) {
